...
@TestModule(name = "test~echo0~1.0")
public class SimpleModuleTest implements PlatformManagerAware {
	...
	@TestModules({ @TestModule(name = "test~echo0~1.0"),
			...
			@TestModule(name = "test~echo2~1.0") })
	public void test1() {
	...
	@Test
	@TestModule(name = "test~echo1~1.0", instances = 3)
	public void test2() {
		int instances = platformManager.listInstances().size();
	...
	@Test
	@TestModule(name = "test~echo2~1.0", jsonConfig = "{\"port\":8091}")
	public void test3() {
		int instances = platformManager.listInstances().size();
